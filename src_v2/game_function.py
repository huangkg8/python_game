...
def handle_keydown(sett,screen,event,hero,bullets):
	...
	#开敌机		
		...
		#瞄准，开火
			...
			#离开敌机
			hero.rect.centerx,hero.rect.centery=hero.plane.rect.centerx,hero.plane.rect.centery
			hero.plane=None
			hero.jumping=True
...
def update_bullets(sett,bullets,hero,enemies):
	...
	for i in range(3):
		...
		if collisions0:
			for enemy_hit in collisions0.values():
				enemy_hit.life-=sett.lethality[i]
				if enemy_hit.life<=0:
	...
	for i in range(3):
		...
		if collisions1:
			for mymissile,enmbullet in collisions1.items():			
				mymissile.life-=lethality[i]
				if mymissile.life<=0:
					bullets[2].remove(mymissile)
				if i==2:
					enmbullet.life-=lethality[2]
					if enmbullet.life<=0:
	...
	for i in range(3):
		...
		if collisions2:
			for enmmissile,mybullet in collisions2.items():			
				enmmissile.life-=lethality[i]
				if enmmissile.life<=0:
					bullets[5].remove(enmmissile)
				if i==2:
					mybullet.life-=lethality[2]
					if mybullet.life<=0:
